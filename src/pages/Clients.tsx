...
export default function Clients() {
  ...
  const addCliente = () => {
    ...
    setClientes((prev) => [...prev, { ...nuevo, id: `c-${String(prev.length + 1).padStart(3, "0")}` }]);
    ...
    <section className="max-w-5xl mx-auto mt-10 space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-gray-600">Gestión básica: listado y alta rápida (datos en memoria).</p>
        </div>
      </header>
      ...
      <div className="bg-white shadow rounded p-6">
        ...
        <div className="grid md:grid-cols-4 gap-3">
          <input className="border rounded px-3 py-2" placeholder="Nombre" value={nuevo.nombre}
                 onChange={(e) => setNuevo({ ...nuevo, nombre: e.target.value })} />
          <input className="border rounded px-3 py-2" placeholder="Email" value={nuevo.email}
                 onChange={(e) => setNuevo({ ...nuevo, email: e.target.value })} />
          <input className="border rounded px-3 py-2" placeholder="Teléfono (opcional)" value={nuevo.telefono}
